...
if (APP_MODE === "personal") {
    ...
    setInterval(() => {
        checkPersonaSchedules().catch((e) => console.error("[日程定时器]", e.message));
    }, 60 * 1000);
    ...
    setTimeout(async () => {
        try {
            ...
            if (data) {
                data.forEach((row) => {
                    if (row.key === "api_key" && row.value && row.value.trim() !== "undefined")
                        process.env.AI_API_KEY = row.value;
                    if (row.key === "api_base_url" && row.value && row.value.trim() !== "undefined" && row.value.startsWith("http"))
                        process.env.AI_BASE_URL = row.value;
                    if (row.key === "api_model" && row.value && row.value.trim() !== "undefined") {
            ...
            if (subData) {
                subData.forEach((row) => {
                    if (row.key === "sub_api_key" && row.value && row.value.trim() !== "undefined")
                        process.env.AI_SUB_API_KEY = row.value;
                    if (row.key === "sub_api_base_url" && row.value && row.value.trim() !== "undefined" && row.value.startsWith("http"))
                        process.env.AI_SUB_BASE_URL = row.value;
                    if (row.key === "sub_api_model" && row.value && row.value.trim() !== "undefined")
                        process.env.AI_SUB_MODEL = row.value;
                });
                console.log("已从数据库安全加载副 API 配置, 模型:", process.env.AI_SUB_MODEL);
            }
        } catch (e) {
